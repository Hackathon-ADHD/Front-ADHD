...
export const kakaoLogin = async (authorizationCode) => {
    try {
        const response = await axios.get(`${BASE_URL}/login/kakao?code=${authorizationCode}`, {});

        const { accessToken, refreshToken, email, newMember } = response.data.body.data;
...
export const naverLogin = async (authorizationCode) => {
    try {
        const response = await axios.get(`${BASE_URL}/login/naver?code=${authorizationCode}`, {});
